...
- **The columns themselves are unmapped, not dropped.** `entrypoint.sh` runs
  ...
  *previous* Cloud Run revision is still serving traffic. Dropping columns that
  ...
  maps them.
- **Accepted: a blank served by the outgoing revision during cutover is lost.**
  It writes only the legacy columns, which the new code no longer reads. A
  three-release dual-write would close this and was specified, then declined on
  proportionality — see design.md. Deploy when nobody is mid-night, and note the
  window in the release notes so a lost blank is recognised rather than
  investigated.
