...
CommandError even though the schema ended up correct. This retries a few
times, tolerating exactly that race while still surfacing genuine failures.
"""
...
from app import _UPGRADE_ATTEMPTS, _upgrade_with_race_retry


def _retry(upgrade_fn):
    sleeps = []
    _upgrade_with_race_retry(upgrade_fn, sleep=sleeps.append)
    return sleeps


def test_succeeds_immediately_when_no_race():
    upgrade_fn = MagicMock()
    assert _retry(upgrade_fn) == []
...
def test_losing_again_on_the_next_revision_is_retried_too():
    """A deploy carrying two migrations: the loser loses the first bookkeeping
    update, retries while the winner is applying the second, and loses that one
    too. One retry was not enough; the two-process migration test showed it."""
    upgrade_fn = MagicMock(side_effect=[SystemExit(1), SystemExit(1), None])
    assert _retry(upgrade_fn) == [0.5, 1.0]
    assert upgrade_fn.call_count == 3
...
def test_genuine_failure_still_raises_after_retry():
    ...
    with pytest.raises(RuntimeError):
        _retry(upgrade_fn)
    assert upgrade_fn.call_count == _UPGRADE_ATTEMPTS
